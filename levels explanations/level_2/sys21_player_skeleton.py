...
def start_attacking():
    ...
    for t_i in range(4, 4096 + 1):
        if '''Complete the missing line''' : #Check proposition, read line 13.
            continue
    ...
    if lcm_t > 1:
        ...
        while l_bound <= r_bound:
            ...
            for j in range(0, sb + 1):
                if '''Complete''' and gmpy2.gcd(middle - j, lcm_t) == 1: #Check propositon conditions
    ...
    while not (len(M) == 1 and M[0][0] == M[0][1]):
        ...
        if counter == 1:
            ...
            while not flag_s:
                j += 1
                for s_cand in '''Complete the missing line''': #Deduce a smaller range by skipping holes theorem.
